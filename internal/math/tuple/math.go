...
func (t Tuple) Equal(t2 Tuple) bool {
	return mathUtil.Equals(t.x, t2.x) &&
		mathUtil.Equals(t.y, t2.y) &&
		mathUtil.Equals(t.z, t2.z) &&
		mathUtil.Equals(t.w, t2.w)
}
...
// Reflect returns the result of reflecting the vector around the other vector.
func (t Tuple) Reflect(t2 Tuple) Tuple {
	if !(t.IsVector() && t2.IsVector()) {
		panic("invalid operand types - must be a vectors")
	}

	return t.Sub(t2.Mul(2.0).Mul(t.Dot(t2)))
}
